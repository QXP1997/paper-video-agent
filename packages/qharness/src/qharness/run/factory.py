...
def create_loop_services(
    ...
    actor = Actor(model, tools)
    verifier = VerificationController(CheckRunner(tools), model)
    return LoopServices(repository, model, tools, actor, verifier, Executor(actor, verifier))
