...
def get_tts_config(
    ...
    return {
        "backend": "edge",
        "voice": (
            voice or os.getenv("PAPER_VIDEO_TTS_VOICE", DEFAULT_TTS_VOICE)
        ).strip(),
        "rate": (
            rate or os.getenv("PAPER_VIDEO_TTS_RATE", DEFAULT_TTS_RATE)
        ).strip(),
        "pitch": (
            pitch or os.getenv("PAPER_VIDEO_TTS_PITCH", DEFAULT_TTS_PITCH)
        ).strip(),
    }
